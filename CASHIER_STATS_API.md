# 📊 Топ Кассиров - API Статистика

## Обзор

Эндпоинт для получения статистики по кассирам, отсортированной по объему продаж (топ-исполнители).

---

## 🔗 Эндпоинт

```
GET /api/sales/sessions/cashier-stats/
```

или

```
GET /api/sales/shifts/cashier-stats/
```

*(оба URL работают одинаково, `shifts` - это алиас для `sessions`)*

---

## 📋 Query Parameters

| Параметр | Тип | Обязательный | По умолчанию | Описание |
|----------|-----|--------------|--------------|----------|
| `date_from` | string (ISO datetime) | Нет | Начало текущего месяца | Дата начала периода |
| `date_to` | string (ISO datetime) | Нет | Сейчас | Дата окончания периода |
| `limit` | integer | Нет | Без ограничений | Количество топ-кассиров для вывода |

**Формат даты:** `YYYY-MM-DDTHH:MM:SS` или `YYYY-MM-DD`

---

## 📤 Response Format

```json
{
  "status": "success",
  "data": {
    "period": {
      "from": "2025-11-01T00:00:00+05:00",
      "to": "2025-11-30T23:59:59+05:00"
    },
    "cashiers": [
      {
        "id": 8,
        "full_name": "Петров Иван",
        "phone": "+998901234568",
        "role": "cashier",
        "total_sales": "1500000.00",
        "cash_sales": "0",
        "card_sales": "1500000.00",
        "sales_count": 1,
        "sessions_count": 1
      },
      {
        "id": 7,
        "full_name": "Иванов Антон",
        "phone": "+998901234567",
        "role": "cashier",
        "total_sales": "1500000.00",
        "cash_sales": "1500000.00",
        "card_sales": "0",
        "sales_count": 2,
        "sessions_count": 1
      }
    ],
    "total_cashiers": 2
  }
}
```

---

## 📊 Поля Response

### Period
- `from` - Начало периода (ISO datetime с timezone)
- `to` - Конец периода (ISO datetime с timezone)

### Cashiers (массив)
- `id` - ID кассира (Employee)
- `full_name` - Полное имя кассира
- `phone` - Номер телефона
- `role` - Роль (`cashier`, `stockkeeper`)
- `total_sales` - Общая сумма продаж (Decimal)
- `cash_sales` - Сумма продаж наличными (Decimal)
- `card_sales` - Сумма продаж по карте (Decimal)
- `sales_count` - Количество продаж
- `sessions_count` - Количество уникальных смен

### Meta
- `total_cashiers` - Общее количество кассиров в списке

---

## 💡 Примеры использования

### 1. Топ 10 кассиров за текущий месяц

```bash
curl -X GET "http://localhost:8000/api/sales/sessions/cashier-stats/?limit=10" \
  -H "Authorization: Bearer $TOKEN" \
  -H "X-Tenant-Key: test_shop_4dfa7a5a"
```

### 2. Топ 5 кассиров за определенный период

```bash
curl -X GET "http://localhost:8000/api/sales/sessions/cashier-stats/?date_from=2025-11-01T00:00:00&date_to=2025-11-30T23:59:59&limit=5" \
  -H "Authorization: Bearer $TOKEN" \
  -H "X-Tenant-Key: test_shop_4dfa7a5a"
```

### 3. Все кассиры за сегодня

```bash
curl -X GET "http://localhost:8000/api/sales/sessions/cashier-stats/?date_from=2025-11-19T00:00:00&date_to=2025-11-19T23:59:59" \
  -H "Authorization: Bearer $TOKEN" \
  -H "X-Tenant-Key: test_shop_4dfa7a5a"
```

### 4. Топ 3 кассира за последние 7 дней

```bash
# Получаем дату 7 дней назад
DATE_FROM=$(date -u -d '7 days ago' +%Y-%m-%dT00:00:00)
DATE_TO=$(date -u +%Y-%m-%dT23:59:59)

curl -X GET "http://localhost:8000/api/sales/sessions/cashier-stats/?date_from=$DATE_FROM&date_to=$DATE_TO&limit=3" \
  -H "Authorization: Bearer $TOKEN" \
  -H "X-Tenant-Key: test_shop_4dfa7a5a"
```

---

## 🔍 Логика работы

1. **Фильтрация:**
   - Учитываются только завершенные продажи (`status='completed'`)
   - Учитываются только продажи с назначенным кассиром (`cashier__isnull=False`)
   - Фильтр по дате применяется к полю `created_at` продажи

2. **Агрегация:**
   - Группировка по кассиру (`cashier__id`)
   - Подсчет общей суммы продаж (`total_amount`)
   - Подсчет количества продаж
   - Подсчет уникальных смен

3. **Способы оплаты:**
   - Для каждого кассира отдельно подсчитывается сумма наличных (`cash`)
   - И сумма безналичных платежей (`card`)

4. **Сортировка:**
   - По убыванию `total_sales` (топ-исполнители первыми)

5. **Ограничение:**
   - Применяется `limit` если указан в параметрах

---

## 🎯 Use Cases

### Для менеджера
- Определить лучших кассиров за период
- Сравнить производительность кассиров
- Анализ предпочитаемых способов оплаты каждого кассира

### Для владельца
- KPI отчеты по кассирам
- Планирование премий и бонусов
- Выявление слабых мест в работе кассиров

### Для фронтенда
- Dashboard с топ-5 кассиров за день/неделю/месяц
- Графики производительности
- Leaderboard (таблица лидеров)

---

## ⚠️ Важные замечания

1. **Новая логика с Sale.cashier:**
   - Статистика теперь основана на поле `Sale.cashier`, а не `CashierSession.cashier`
   - Это позволяет точно отслеживать, кто именно сделал каждую продажу
   - Несколько кассиров могут работать на одной смене одновременно

2. **Миграция данных:**
   - Старые продажи без `cashier` не будут учитываться в статистике
   - Только продажи, созданные после добавления поля `Sale.cashier`, будут учтены

3. **Производительность:**
   - Запрос оптимизирован с использованием агрегации Django ORM
   - Использует `Coalesce()` для обработки NULL значений
   - Минимальное количество запросов к БД (2 запроса: основной + платежи)

4. **Timezone:**
   - Все даты хранятся и возвращаются с учетом timezone сервера
   - При фильтрации учитывается timezone

---

## 🔐 Требования

- **Аутентификация:** Bearer token (JWT)
- **Заголовок:** `X-Tenant-Key` с ключом магазина
- **Права:** Любой авторизованный пользователь магазина (включая staff)

---

## 📝 Связанные файлы

- **View:** `/sales/views.py:288-374` (`CashierSessionViewSet.cashier_stats()`)
- **Model:** `/sales/models.py` (Sale, Payment, Employee)
- **URL:** `/sales/urls.py:16` (router для sessions/shifts)

---

**Дата создания:** 2025-11-19
**Автор:** Claude Code
**Версия:** 1.0
